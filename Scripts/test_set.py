import cv2
import random,os,cv2,shutil #importing the required libraries
    ...
    for directory in (parasite_dir, uninfected_dir):
        ...
        for path in random.sample(os.listdir(directory),samples):  
            ...
            if os.path.isfile(img_path):
                img = cv2.imread(img_path) #reading the image path and storing it 
                img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB) #open cv reads image in BGR format so converting it into RGB for further use
                img = cv2.resize(img, target_size) #converting the images into the required size
                shutil.move(img_path, target_dir) #transferred the image from source dir to the target dir
            
        print(f'Taken 100 samples from {directory} and shifted it to {target_dir}')
            
        
    print('Total number of samples in the target directory is {}'.format(len(os.listdir(r'..\holdout_dataset')))) #printing the total number of images in holdout_dataset folder
